...
const ManagerProcessReturn: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        let form = await getInspectionFormByBooking(bookingId, 'inleveren');
        if (!form) {
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      // Title
      doc.setFontSize(16);
      doc.text('AFLEVERINGSBEWIJS AUTO', pageWidth / 2, yPosition, { align: 'center' });
      yPosition += 15;
      ...
      doc.setFontSize(12);
      doc.text('INSPECTIEGEGEVENS INLEVEREN', 20, yPosition);
      yPosition += 8;
      doc.setFontSize(10);

      if (inspectionForm.odometer_reading_start) {
        doc.text(`Kilometerstand Begin: ${inspectionForm.odometer_reading_start} km`, 25, yPosition);
        yPosition += 6;
      }

      if (inspectionForm.odometer_reading_end) {
        doc.text(`Kilometerstand Einde: ${inspectionForm.odometer_reading_end} km`, 25, yPosition);
        yPosition += 6;
      }
      ...
      // Download PDF
      doc.save(`Afleveringsbewijs_${booking.id}_${new Date().getTime()}.pdf`);
  ...
  const handleSaveAndComplete = async () => {
    ...
    try {
      ...
      showSuccess('Inleveren succesvol geregistreerd');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    return (
      <div className="flex h-screen" style={{ background: 'var(--vl-bg-primary)' }}>
        ...
        <div className="flex-1 flex flex-col overflow-hidden lg:pl-64">
          <Header title="Inleveren Verwerken" />
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="mt-6 flex gap-3 justify-end">
              ...
              >
                ...
                {isGeneratingPDF ? 'Generen...' : 'Download Afleveringsbewijs PDF'}
              </button>
            </div>
          </div>
        </main>
...
export default ManagerProcessReturn;
