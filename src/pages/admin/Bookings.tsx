...
const AdminBookings: React.FC = () => {
  ...
  const handleCancel = async (booking: Booking) => {
    await showConfirm(
      ...
      async () => {
        try {
          await cancelBooking(booking.id, 'Geannuleerd door beheerder');
  ...
  const handleApproveCancellationRequest = async (booking: Booking) => {
    ...
    await showConfirm(
      ...
      async () => {
        try {
          await cancelBooking(booking.id, `Opzegging goedgekeurd door beheerder (aangevraagd door klant)`);
          await confirmCancellation(booking.id, user.uid);
  ...
  const getAvailableActions = (booking: Booking) => {
    ...
    // Confirmed: needs borg confirm + bezorgkosten + pickup inspection form
