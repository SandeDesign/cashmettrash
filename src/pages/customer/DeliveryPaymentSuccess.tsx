...
const DeliveryPaymentSuccess: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const processPayment = async () => {
      try {
        ...
        if (booking.delivery_cost_confirmed) {
          setStatus('success');
          return;
        }
        ...
        await updateDoc(doc(db, 'bookings', bookingId), {
          ...
          delivery_cost_confirmed_by: 'stripe',
        ...
        setErrorMessage(err.message || 'Fout bij het verwerken van de bezorgkostenbetaling');
        setStatus('error');
      }
    };
  ...
  return (
    <div className="flex h-screen" style={{ background: 'var(--vl-bg-primary)' }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden lg:pl-64">
        <Header title="Bezorgkosten betaling" />
        <main className="flex-1 flex items-center justify-center p-4">
          {status === 'loading' && (
            <div className="text-center">
              <Loader2 className="w-12 h-12 animate-spin text-green-400 mx-auto mb-4" />
              <p className="text-neutral-400">Bezorgkostenbetaling verwerken...</p>
            </div>
          )}
          ...
          {status === 'success' && (
            <div className="vl-card rounded-lg border border-green-500/50 bg-green-900/20 p-8 max-w-md text-center">
              ...
              <h2 className="text-2xl font-bold text-white mb-2">Bezorgkosten betaald!</h2>
              <p className="text-neutral-300 mb-6">
                De bezorgkosten zijn ontvangen. We nemen contact op om de bezorging in te plannen.
              </p>
              <button
...
export default DeliveryPaymentSuccess;
